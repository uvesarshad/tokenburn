...
4. Run with explicit options (you are not in an interactive terminal, so it won't ask): `tokenburn <window> -t <theme> [-n @handle] [--no-cost] --no-preview`
...
6. Finish by giving the user the two commands to use anytime:
   ...
   - `tokenburn` (asks what window and design they want; `tokenburn -y` skips the questions and makes a 30-day card)
